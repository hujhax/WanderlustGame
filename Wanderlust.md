...
Note: for debugging purposes, each minigame has a "query string name".  If a game has a query string name of "foo", and the full game is started with the query parameter "minigame=foo", then the game will start directly with that minigame as if it's stop number 1, and after the post-minigame screen, we just restart that same minigame as stop number 1.  If the query params have `minigame=debug`, instead of getting the intro, you get a menu that lets you select any of the minigame. You can then play the minigame, and when it's done, you go back to the menu. The debug menu includes an "eternal mode" checkbox; if "eternal mode" is checked, we ignore when successes and failures run out, and keep playing the minigame forever (looping content such as the golf holes, climbatorium walls, math levels, or refilling the jeopardy board when content runs out). Otherwise, the game runs normally.
